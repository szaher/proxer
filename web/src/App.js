...
function AdminPlansPage({ api }) {
    ...
    return (_jsxs(_Fragment, { children: [_jsxs(Section, { title: "Create Plan", children: [_jsxs("form", { className: "inline-form", onSubmit: createPlan, children: [_jsx("input", { name: "id", placeholder: "id", required: true }), _jsx("input", { name: "name", placeholder: "name", required: true }), _jsx("input", { name: "max_routes", type: "number", min: 1, placeholder: "max routes", required: true }), _jsx("input", { name: "max_connectors", type: "number", min: 1, placeholder: "max connectors", required: true }), _jsx("input", { name: "max_rps", type: "number", min: 1, placeholder: "max rps", required: true }), _jsx("input", { name: "max_monthly_gb", type: "number", min: 1, placeholder: "max monthly gb", required: true }), _jsxs("label", { className: "checkbox", children: [_jsx("input", { type: "checkbox", name: "tls_enabled" }), "TLS enabled"] }), _jsx("button", { type: "submit", children: "Save" })] }), message ? _jsx("p", { className: "status", children: message }) : null] }), _jsxs(Section, { title: "Plans", actions: _jsx("button", { onClick: () => void load(), children: "Refresh" }), children: [loading ? _jsx("p", { children: "Loading..." }) : null, error ? _jsx("p", { className: "status error", children: error }) : null, !loading && !error ? (_jsxs("table", { children: [_jsx("thead", { children: _jsxs("tr", { children: [_jsx("th", { children: "ID" }), _jsx("th", { children: "Name" }), _jsx("th", { children: "Routes" }), _jsx("th", { children: "Connectors" }), _jsx("th", { children: "RPS" }), _jsx("th", { children: "Monthly GB" }), _jsx("th", { children: "TLS" })] }) }), _jsx("tbody", { children: plans.length === 0 ? (_jsx("tr", { children: _jsx("td", { colSpan: 7, children: "No plans." }) })) : (plans.map((plan) => (_jsxs("tr", { children: [_jsx("td", { children: plan.id }), _jsx("td", { children: plan.name }), _jsx("td", { children: plan.max_routes }), _jsx("td", { children: plan.max_connectors }), _jsx("td", { children: plan.max_rps }), _jsx("td", { children: plan.max_monthly_gb }), _jsx("td", { children: _jsx(Badge, { value: plan.tls_enabled ? "enabled" : "disabled" }) })] }, plan.id)))) })] })) : null] })] }));
}
...
function Workspace({ me, api, onLogout, }) {
    const isSuper = me.user.role === "super_admin";
...
export function App() {
    ...
    if (checkingAuth) {
        return _jsx("main", { className: "auth-shell", children: "Checking session..." });
    }
    if (!me) {
        return _jsx(LoginView, { onLoggedIn: refreshSession });
    }
    return _jsx(Workspace, { me: me, api: api, onLogout: logout });
}
